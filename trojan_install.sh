...
ubuntu_tojan_install() {
  ...
  trojan_bin_location='/usr/local/bin/'
  trojan_service_location='/etc/systemd/system/'
...
auto_config_modify() {
  ...
  echo -n "$title[默认:"$(echo $config_orginal | cut -f2 -d'"')"]:"
  read new_config_modify
...
serverSpeeder_install() {
  if [[ ! -f /etc/redhat-release ]] || [[ $(cat /etc/redhat-release | grep -c -i centos) -eq 0 ]]; then
    ...
    cat /etc/redhat-release
    if [[ $(uname -r | grep -ci 3.10.0-229.1.2.el7.x86_64) -eq 0 ]]; then
      echo -n "未安装支持锐速内核! 是否安装锐速内核y/n[默认:y] : "
      read i
      if [ x$i == x ]; then
        i='y'
      fi
